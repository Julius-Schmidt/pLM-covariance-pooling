...
class TestCovariancePoolingMaskInvariance:
    ...
    def test_padding_does_not_change_result(self, extra):
        ...
        assert torch.allclose(ref, out, atol=1e-5), (
            f"CovariancePooler output changed with {extra} padding rows."
        )


class TestHybridPoolingMaskInvariance:
    @pytest.mark.parametrize("extra", [1, 5, 20])
    def test_padding_does_not_change_result(self, extra):
        torch.manual_seed(2)
        d, dc, L = 16, 4, 12
        cov = CovariancePooler(d, dc)
        pooler = HybridPooler(d, cov)
        X = torch.randn(L, d)
        mask_full = torch.ones(L, dtype=torch.bool)
        ...
        assert torch.allclose(ref, out, atol=1e-5), (
            f"HybridPooler output changed with {extra} padding rows."
        )
